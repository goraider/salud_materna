...
export class FormComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'nombre', 'nivelAtencion', 'actions'];
  dataSourceClues;
  valorclue:any = [];

  data = Object.assign( this.valorclue );
      ...
      forkJoin(httpCalls).subscribe(
        results => {
          ...
          if(results[2]){
            ...
            this.dataSourceClues = new MatTableDataSource<Element>(this.data);
            this.dataSourceClues =  results[2].clues;
  ...
  agregarClue( event: MatAutocompleteSelectedEvent ){
    ...
    if(this.valorclue.length == 0 && this.valorclue.length < 1){
  ...
  borrarClue(e: any){
    ...
    for(var i = this.dataSourceClues.length - 1; i >= 0; i--) {
        
        this.dataSourceClues.splice(e, 1);
        this.dataSourceClues = new MatTableDataSource<Element>(this.data);

        this.usuarioForm.get('clues').setValue([]);
  ...
  guardarUsuario(){
    ...
    if(this.dataSourceClues.length > 0){
